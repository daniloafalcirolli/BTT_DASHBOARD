...
import { RECALCULO_VALOR_COMBUSTIVEL } from '../../../services/funcionario/FuncionarioService';
...
const ModalRecalculo = ( props ) => {
		...
		if(dataInicio !== "" && dataFinal !== ""){
			if(new Date(dataInicio) <= new Date(dataFinal)){
				...
				if(window.confirm("Tem certeza que deseja alterar as rotas desse funcionário entre o dia " + dataInicio + " e o dia " + dataFinal + "?")){
					const { url, options } = RECALCULO_VALOR_COMBUSTIVEL({ nome_cidade: props.cidade.value, data_inicio: dataInicio, data_fim: dataFinal});
	...
	return (
		<>
			<button className='btn btn-primary' onClick={toggleModal}>Recalcule o valor do combustível das rotas</button>
			{
					...
					<>
						<div className="col-12">
							<form onSubmit={handleSubmit} className="row">
								...
								<div className="col-12">
									<p>Obs.: Salve o valor do combustível nessa cidade antes de alterar o valor das rotas
									</p>
								</div>
