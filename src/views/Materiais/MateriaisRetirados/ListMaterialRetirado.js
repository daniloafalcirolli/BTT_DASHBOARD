...
const MateriaisRetirados = () => {
    ...
    const handleAlterSerial = async (id) => {
        let resp = await MaterialRetiradoService.alterSerial(id);
        if(resp.status === 200){
            return true
        }
    };
    ...
    const handleDelete = async (event) => {
        MaterialRetiradoService.delete(event.currentTarget.id)
        .then((resp) => {
            if(resp.status == 200){
	...
	const getAll = async () => {
        MaterialRetiradoService.getAll(pagination)
        .then((resp) => {
            setData(resp);
        });
	}
...
export default MateriaisRetirados