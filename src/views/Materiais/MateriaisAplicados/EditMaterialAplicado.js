...
const EditMaterialAplicado = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    MaterialAplicadoService.edit({ id: params.id, material: material.value, id_senior: id_senior.value, has_serial: serial })
    .then((resp) => {
      if(resp.status == 200){
  ...
  const getById = () => {
    MaterialAplicadoService.getById(params.id)
    .then((resp) => {
...
export default EditMaterialAplicado;